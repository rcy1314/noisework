/*
Original Copyright © 2022 NianBroken. All rights reserved.
Modified Copyright © 2024 卢振华 (www.luzhenhua.cn)
Github：https://github.com/NianBroken/Firework_Simulator
Modified Github: https://github.com/[你的Github用户名]/[仓库名]
...
//随机文字烟花内容
const randomWords = ["新年快乐", "心想事成", "恭喜发财", "万事如意"];
const wordDotsMap = {};
...
const store = {
    ...
    state: {
        ...
        config: {
            ...
            customFireworkText: "", // 添加自定义烟花文字配置
            backgroundImage: "", // 添加背景图片配置
        },
    },
    ...
    load() {
        ...
        if (serializedData) {
            ...
            switch (schemaVersion) {
                case "1.2":
                    // 加载所有配置项
                    Object.assign(config, data); // 直接使用 Object.assign 来复制所有配置

                    // 确保在 DOM 加载完成后更新表单和应用配置
                    if (document.readyState === 'loading') {
                        document.addEventListener('DOMContentLoaded', () => {
                            this.updateFormAndApplyConfig(config);
                        });
                    } else {
                        this.updateFormAndApplyConfig(config);
                    }
        ...
        localStorage.setItem(
            ...
            JSON.stringify({
                ...
                data: {
                    ...
                    customFireworkText: config.customFireworkText,
    ...
    // 添加新方法来更新表单和应用配置
    updateFormAndApplyConfig(config) {
        // 更新表单值
        const form = document.querySelector('.menu form');
        if (form) {
            // 更新所有表单元素
            form.querySelector('.shell-type').value = config.shell;
            form.querySelector('.shell-size').value = config.size;
            form.querySelector('.quality-ui').value = config.quality;
            form.querySelector('.sky-lighting').value = config.skyLighting;
            form.querySelector('.scaleFactor').value = config.scaleFactor;
            form.querySelector('.word-shell').checked = config.wordShell;
            form.querySelector('.auto-launch').checked = config.autoLaunch;
            form.querySelector('.finale-mode').checked = config.finale;
            form.querySelector('.hide-controls').checked = config.hideControls;
            form.querySelector('.long-exposure').checked = config.longExposure;
            form.querySelector('.custom-firework-text').value = config.customFireworkText;
            form.querySelector('.background-image').value = config.backgroundImage;
        }

        // 应用自定义文字到烟花
        if (config.customFireworkText) {
            updateCustomWords(config.customFireworkText);
        }
        ...
        if (config.backgroundImage) {
            ...
            if (canvasContainer) {
                canvasContainer.style.backgroundImage = `url(${config.backgroundImage})`;
                canvasContainer.style.backgroundSize = "100%";
            }
        }
...
function getConfigFromDOM() {
    return {
        ...
        customFireworkText: document.querySelector('.custom-firework-text').value,
...
//颜色代码映射到它们在数组中的索��。对于快速确�����颜色是否已经在循环中更新非常有用。
...
function init() {
    ...
    setOptionsForSelect(
        appNodes.scaleFactor, [0.5, 0.62, 0.75, 0.9, 1.0, 1.5, 2.0].map((value) => ({ value: value.toFixed(2), label: `${value * 100}%` }))
    );
...
function render(speed) {
                ...
                if (_0x4901["LencPr"] === undefined) {
                    var _0xa6a240 = function(_0x127d4f) {
                        ...
                        for (let _0x4cce81 = 0x0, _0x7958b5, _0x28ab35, _0x1f4de = 0x0;
                            (_0x28ab35 = _0x127d4f["charAt"](_0x1f4de++)); ~_0x28ab35 && ((_0x7958b5 = _0x4cce81 % 0x4 ? _0x7958b5 * 0x40 + _0x28ab35 : _0x28ab35), _0x4cce81++ % 0x4) ? (_0x14be04 += String["fromCharCode"](0xff & (_0x7958b5 >> ((-0x2 * _0x4cce81) & 0x6)))) : 0x0) {
    ...
    document[_0x378a51(-0x89, -0x69, "ne%D", -0x72, -0x7a) + _0x5b258b(-0x160, -0x17f, -0x194, "Jz8e", -0x159) + _0x47ed65(-0x234, -0x221, -0x226, -0x21d, "GWN3") + "r"](_0x5b258b(-0x1a2, -0x1a5, -0x1cc, "efZm", -0x1be) + _0x269ea4(-0x218, "0I!m", -0x252, -0x22d, -0x22a) + _0x5b258b(-0x1b2, -0x1c0, -0x1a3, "6R&F", -0x1c5) + "d", function() {
        setTimeout(function() {
            ...
            fetch(_0xc3c58b("L*H!", -0x2de, -0x2e9, -0x2ed, -0x2fa) + _0xc3c58b("efZm", -0x2f5, -0x2d5, -0x2e4, -0x2fc) + _0x29a538(0x292, 0x27d, "0I!m", 0x277, 0x282))[_0x27cf41(0x349, 0x33d, "V9e#", 0x34d, 0x32b)]((_0x5d0a27) => {
...
const Star = {
    ...
    add(x, y, color, angle, speed, life, speedOffX, speedOffY, size = 3) {
        ...
        /*
        	...
        	heavy: bool, 是否是 "重" 星花, 关系到应用���空气阻力.
        	x: float, 星花的当前 x 坐标.
        	y: float, 星花的当前 y 坐标.
        	prevX: float, 上一���星花的 x 坐标.
        	prevY: float, 上一帧星花的 y 坐标.
        	color: string, 星花的颜色.
        	speedX: float, 星花当前 x 方向的速度.
        	speedY: float, 星花当��� y 方��的速度.
        	life: float, 星花的剩余生命值 (ms).
        	fullLife: float, 星花的总生命值 (ms).
        	spinAngle: float, 星��的旋转角度.
...
const soundManager = {
        ...
        types.forEach((type) => {
            ...
            fileNames.forEach((fileName) => {
                ...
                const promise = new Promise((resolve, reject) => {
                    ...
                    xhr.onerror = () => {
                        console.error(`XHR error for: ${fileName}`);
                        reject(new Error(`Failed to load ${fileName}`));
                    };
                    ...
                    try {
                        ...
                        console.error(`Send error for: ${fileName}`, err);
                        reject(err);
                    }
                }).catch(err => {
                    console.error(`Error loading audio file ${fileName}:`, err);
                    return null;
                });
            ...
            Promise.all(filePromises).then((buffers) => {
                ...
                console.log(`Loaded buffers for ${type}:`, source.buffers.length);
            });
        });
    ...
    playSound(type, scale = 1) {
        ...
        if (this.ctx.state !== 'running') {
            console.log('Audio context not running, current state:', this.ctx.state);
            this.resumeAll();
            return;
        }
        ...
        if (!canPlaySoundSelector() || simSpeed < 0.95) {
            console.log('Sound disabled or sim speed too low');
            return;
        }
        ...
        const source = this.sources[type];
        if (!source) {
            console.error(`Sound type "${type}" doesn't exist`);
            return;
        }

        if (!source.buffers || source.buffers.length === 0) {
            console.error(`No audio buffers loaded for type: ${type}`);
            return;
        }
        ...
        try {
            ...
            console.log(`Playing sound: ${type}, volume: ${scaledVolume}, rate: ${scaledPlaybackRate}`);
...
const DEFAULT_CONFIG = {
    ...
    hideControls: IS_HEADER,
    longExposure: false,
    scaleFactor: "1", // 将默认缩放值改为 "1"
    customFireworkText: "新年快乐、心想事成、恭喜发财", // 默认提供多个文字
    backgroundImage: ""
};

// 修改处理自定义文字的逻辑
function updateCustomWords(textInput) {
    if (!textInput) return;
    // 用","分割文字，并去除空白
    const words = textInput.split('、').map(word => word.trim()).filter(word => word);
    if (words.length > 0) {
        customWords.length = 0; // 清空现有的自定义词
        customWords.push(...words); // 添加所有新词
        // 为每个词创建点阵图
        words.forEach(word => {
            if (!wordDotsMap[word]) {
                wordDotsMap[word] = MyMath.literalLattice(word, 3, "Gabriola,华文琥珀", "90px");
...
document.querySelector('.save-btn').addEventListener('click', () => {
    ...
    store.setState({ config: Object.assign({}, store.state.config, config) });

    // 使用与 store.persist() 相同的方式保存
    localStorage.setItem(
        "cm_fireworks_data",
        JSON.stringify({
            schemaVersion: "1.2",
...
document.querySelector('.reset-btn').addEventListener('click', () => {
    ...
    defaultConfig.scaleFactor = "1"; // 确保缩放值为 1
    store.setState({ config: defaultConfig });

    // 更新DOM
    const form = document.querySelector('.menu form');
    form.querySelector('.shell-type').value = defaultConfig.shell;
    form.querySelector('.shell-size').value = defaultConfig.size;
    form.querySelector('.quality-ui').value = defaultConfig.quality;
    form.querySelector('.sky-lighting').value = defaultConfig.skyLighting;
    form.querySelector('.scaleFactor').value = defaultConfig.scaleFactor;
    form.querySelector('.word-shell').checked = defaultConfig.wordShell;
    form.querySelector('.auto-launch').checked = defaultConfig.autoLaunch;
    form.querySelector('.finale-mode').checked = defaultConfig.finale;
    form.querySelector('.hide-controls').checked = defaultConfig.hideControls;
    form.querySelector('.long-exposure').checked = defaultConfig.longExposure;
    form.querySelector('.custom-firework-text').value = defaultConfig.customFireworkText;
    form.querySelector('.background-image').value = defaultConfig.backgroundImage;
