...
JSNES.Keyboard.prototype = {
    ...
    keyDown: function(evt) {
        ...
        //按键  0,1  玩家2 连发和连跳
        if (evt.keyCode == 96 || evt.keyCode == 101) { // 按键 0 1，连发和连跳
    		var keyno;
    		if (evt.keyCode == 96)
    			keyno = this.keys.KEY_B;
    			...
    			keyno = this.keys.KEY_A;
		   	setTimeout(function() {
				that.state2[keyno] = that.state2[keyno] != 0x41 ? 0x41 : 0x40;
			}, 100);
    ...
    keyUp: function(evt) {
        ...
        //玩家2
        if (evt.keyCode == 96 || evt.keyCode == 101) { // 按键 U I，连发和连跳
    		if (evt.keyCode == 96)
    			this.setKey(105, 0x40); // KEY_B-J-74
    		else if (evt.keyCode == 101)
    			this.setKey(103, 0x40); // KEY_A-K-75
    		if (evt.preventDefault) {
	            evt.preventDefault();
	        }
    		return;
